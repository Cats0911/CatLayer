...
namespace CatLayer
{
    ...
    internal sealed class OverlayItemForm : Form
    {
        ...
        public OverlayItemForm(MainForm app, ItemType type, string data, int seconds, int initialOpacityPercent, TimerMode timerMode, string alarmPath, string customName)
        {
            ...
            if (Type == ItemType.Timer)
            {
                ...
                renderTimer.Tick += delegate
                {
                    ...
                    else if (TimerKind != TimerMode.Stopwatch) GetTimerText();
                };
                renderTimer.Start();
            }
            if (Type == ItemType.ObsProgram)
        ...
        protected override void OnSizeChanged(EventArgs e)
        {
            ...
            else if (Type != ItemType.Web) RenderLayered();
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
    ...
    internal sealed class MainForm : Form, IMessageFilter
    {
        ...
        public MainForm()
        {
            ...
            foregroundUiTimer.Interval = 120;
        ...
        private void BuildMainUi()
        {
            ...
            ToolStripMenuItem loadGroupFile = new ToolStripMenuItem("그룹 파일 불러오기..."); loadGroupFile.Click += delegate { LoadGroupInteractive(); };
            ToolStripMenuItem openGroupFolder = new ToolStripMenuItem("그룹 파일 위치 열기"); openGroupFolder.Click += delegate { OpenGroupFolder(); };
            ...
            appMenu.Items.Add(loadGroupFile);
            appMenu.Items.Add(openGroupFolder);
            ...
            addWeb.TextImageRelation = TextImageRelation.ImageBeforeText; addWeb.Padding = new Padding(8, 0, 5, 0);
            ...
            Button savePreset = NewPanelButton(left, "저장", 18, 340, 239, 40); SetButtonIcon(savePreset, "icon_save.png"); savePreset.Click += delegate { SavePresetInteractive(); };
            Button loadPreset = NewPanelButton(left, "불러오기", 18, 386, 239, 40); SetButtonIcon(loadPreset, "icon_load.png"); loadPreset.Click += delegate { LoadPresetInteractive(); };
            Button deletePreset = NewPanelButton(left, "프리셋 삭제", 18, 432, 239, 40, true); SetButtonIcon(deletePreset, "icon_trash.png"); deletePreset.Click += delegate { DeletePresetInteractive(); };

            NewSeparator(left, 18, 488, 239);
            NewLabelOn(left, "전체 제어", 18, 502, 220, 26, 12F, FontStyle.Bold, UiText);
            Button clearAll = NewPanelButton(left, "전체 삭제", 18, 536, 112, 34, true); clearAll.Click += delegate { ClearAllInteractive(); };
            Button undo = NewPanelButton(left, "실행 취소", 145, 536, 112, 34); undo.Click += delegate { UndoLastAction(); };
            Button reset = NewPanelButton(left, "설정 초기화", 18, 578, 112, 34); reset.Click += delegate { ResetSettingsInteractive(); };
            editButton.SetBounds(145, 578, 112, 34); StyleButton(editButton, false); editButton.Click += delegate { CycleEditorMode(); }; left.Controls.Add(editButton);
            ...
            overlayListMenu.Items.Add(new ToolStripSeparator());
            overlayListMenu.Items.Add(makeGroup);
            overlayListMenu.Items.Add(saveGroupFile);
            overlayListMenu.Items.Add(loadGroupFromMenu);
            overlayListMenu.Items.Add(breakGroup);
        ...
        private void SetButtonIcon(Button button, string fileName)
        {
            try
            {
                ...
                button.Image = new Bitmap(img, new Size(compact ? 16 : 18, compact ? 16 : 18));
                img.Dispose();
                if (compact)
                {
        ...
        private void ResizeOverlayColumns()
        {
            ...
            overlayList.Columns[2].Width = Math.Max(1, w - c1 - c2);
        ...
        public List<OverlayItemForm> GetGroupMembers(OverlayItemForm source)
        {
            ...
            foreach (OverlayItemForm f in items) if (f.GroupId == source.GroupId) result.Add(f);
        ...
        {
            decimal v = value; if (v < n.Minimum) v = n.Minimum; if (v > n.Maximum) v = n.Maximum; n.Value = v;
        }
        ...
        private void GroupSelectedOverlays()
        {
            List<OverlayItemForm> selected = SelectedOverlays;
        ...
        private void UngroupSelectedOverlays()
        {
            List<OverlayItemForm> selected = SelectedOverlays;
        ...
        public void EnterWebControlFromDoubleClick(OverlayItemForm web)
        {
            ...
            web.FocusWebContent();
            SetStatus("웹 편집 모드  |  웹 바깥 클릭 = 일반 편집 모드");
        }
        ...
        private void SetEditorMode(EditorMode mode, bool save)
        {
            ...
            UpdateButtons();
            if (WebControlMode) SetStatus("웹 조작 모드  |  웹 클릭/스크롤/입력 가능  |  Ctrl+휠 = 웹 확대/축소  |  F10 종료");
            if (save) SaveConfig();
        }
        ...
        {
            obsBridgeLabel.Text = ObsBridge.IsAlive() ? "OBS: Bridge 준비" : "OBS: Bridge 미연결";
        }
        ...
        private void SaveSelectedGroupInteractive()
        {
        ...
        private void LoadGroupInteractive()
        {
            using (OpenFileDialog d = new OpenFileDialog())
            {
        ...
        private void SaveGroupFile(string path, string name, List<OverlayItemForm> groupItems)
        {
            ...
            foreach (OverlayItemForm f in groupItems)
            {
                ...
                lines.Add("ITEM|" + ((int)f.Type).ToString() + "|" + payload + "|" + f.DurationSeconds.ToString() + "|" +
        ...
        public void SaveConfig()
        {
            try
            {
                List<string> lines = new List<string>(); lines.Add("EDIT=" + (EditMode ? "1" : "0"));
